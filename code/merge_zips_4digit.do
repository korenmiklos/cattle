...
set more off
capture log close
log using ../doc/merge_zips_4digit, text replace
...
gen sector = real(substr(naics,1,4))
drop n*

* merge different sectors
collapse (sum) emp est, by(zip sector)

* used to fill in all sectors, not doing it for 4 digits
...
saveold ../data/zip_business_patterns_4digit, replace
