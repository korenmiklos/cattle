...
city_type   =   'circular'; %'circular' or 'linear'
...
for jj=1:no_cit
    ...
    %changing variables to unconstrained from constrained
    z_init_vec0(1,1) = tan(z_vec0(1,1)*pi/(z3(jj,1)-0)-pi/2);
    z_init_vec0(2,1) = tan((z_vec0(2,1)-z_vec0(1,1))*pi/(z3(jj,1)-z_vec0(1,1))-pi/2);
    
    options = optimset('Display','iter');
    [z_init_vec_p, diff_p(1:2,jj), exitf(jj,1)] = fsolve(@z_func,z_init_vec0,options);    
    
%    itmax   =   100;
%    crit    =  1e-6;
%    [z_init_vec_p(1:2,1), exitf(jj,1)] = csolve(@z_func,z_init_vec0,[],crit,itmax);

    z_init_vec(jj,1:2) = z_init_vec_p';

    %changing variables back to constrained
    z_vec(jj,1)   =   (atan(z_init_vec(jj,1))+pi/2)*(z3(jj,1)-0)/pi;
    z_vec(jj,2)   =   z_vec(jj,1)+(atan(z_init_vec(jj,2))+pi/2)*(z3(jj,1)-z_vec(jj,1))/pi;
