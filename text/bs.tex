...
\[
u(m,s,h) = \left[\left(m^{\gamma}m^{1-\gamma}\right)^{1-1/\sigma}+h^{1-1/\sigma}\right]^{\sigma/(\sigma-1)}.
\]
...
\frac{y_m-m_1}{L} = \frac{(y_m+s_1)-(m_1+s_1)}{L} = AF(N_1/L,1-{\lambda}) - AF_L(N_1/L,1-{\lambda}),
\end{equation}
