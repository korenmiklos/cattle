...
\begin{itemize}
    ...
    \item This raises urban rents, raising the relative price of non-tradables.
\end{itemize}
\end{frame}
\time{2}
...
\section{Characterizing the supply side}
...
Assume $A_m/A_s$ is constant. Service price increases if and only if demand for ``urban'' land ($\Bar z$) increases relative to ``rural'' land.
\end{block}
\end{frame}

\time 1

\section{Closing the model}

\begin{frame}\frametitle{Development and urban land}
Demand for urban land may increase for a number of reasons.
\begin{enumerate}
\item \jumpto{PPF-location-nonhomothetic-1}{Structural change shifts demand towards services}.
\item \jumpto{housing}{Development increases the demand for residential land}.
\item \jumpto{PPF-location-transport-1}{Manufacturing shipping costs decline disproportionately}.
\end{enumerate}
Next we develop the housing-based explanation.
\end{frame}
\time 4
...
\begin{itemize}
    \item Consumers have homothetic utility over $m$, $s$ and $h$ (housing services).
...
\begin{itemize}
    ...
    \tau_h>\tau_s>\tau_m.
    \]
    \item $\to$ people like to live close to the CBD.
\end{itemize}
\end{frame}
\time{1}
...
\subsection{Relative prices and development}
...
\subsection{Income and housing demand}
\begin{frame}\frametitle{Question}
How does the demand for residential land vary with income?
\end{frame}

\widefigure{sc_davis}{Land prices and income across large U.S.~cities}
\time 1

\begin{frame}\frametitle{Land prices, demand for housing and income}
\begin{table}[h!]
\center 
...
\begin{frame}\frametitle{Trends in the demand for land}
\begin{itemize}
    \item Over time, the demand for residential land has increased in the U.S.
    \item Overman, Puga and Turner (2006) find that the per capita residential land use between 1976 and 1992 \emph{increased by 25.4\%}.
    \item At the same time, land prices have increased dramatically, so \emph{demand} must have increased.
\end{itemize}    \end{frame}


\widefigure{dh-shares}{Land prices and the share of land in home value\\
\small Source: Davis and Heathcote (2007)}
\time 1
\longfigure{dh-prices}{Real residential and farm land prices (log scale)\\
\small Source: Davis and Heathcote (2007).}
\time 1

\subsection{Industry location}
\begin{frame}\frametitle{Question}
Do nontradable sectors locate closer to residents?
\end{frame}

\widefigure{ntcounties}{The share of non-traded employment in U.S.~counties\\
\small {Source}: 2000 U.S.~Census, Summary File 3.}
\time 1
\widefigure{LQ_scatter}{Locational quotient and population density\newline
 \small Source: 2000 U.S.~Census}
\time 2
\widefigure{deurbanization}{Share of 100 most densely populated counties\newline
\small Source: County Business Patterns}
\time 1

\subsection{Income and relative prices}
\begin{frame}\frametitle{Question}
...
\begin{frame}\frametitle{Where to go from here?}
\begin{itemize}
\item Our model can be used in a number of ways.
\begin{enumerate}
    \item Help us understand \emph{sectoral development accounting}.
    \item Can be extended to study the spatial patterns of prices across a large number of sectors, relating them to development, urbanization patterns, geographical features etc.
\end{enumerate}    
\end{itemize}
\end{frame}
...
\begin{frame}\frametitle{\backto{housing}{Introducing housing}}
\begin{itemize}
    \item We introduce homothetic preferences for housing.
    \item Housing is complementary with consumption:
\[
u(m,s,h) = \min\{m^\gamma s^{1-\gamma} ,h/H\}.
\]
    \item No technical progress in housing (or slower than in manufacturing).
    \item High commuting costs.
\end{itemize}
These assumptions imply that
\begin{enumerate}
    \item residents live closest to CBD,
    \item residential land increases with development.
\end{enumerate}    
\end{frame}
