...
\item With only trade cost differences across sectors, service price increases if and only if land devoted to manufacturing decreases.
\item Why does manufacturing shrink with development?
\end{itemize}
\end{frame}
...
Land devoted to manufacturing may shrink for a number of reasons.
\begin{enumerate}
...
\begin{itemize}
    ...
    \item Commuting costs a $1-\exp(-\tau_h z)$ fraction of the consumption bundle.
    \item So that indirect utility is
    \[
    u(I,p_m,p_s,p_h) = \frac{\exp(-\tau_h z)I}{P[\Phi(p_m,p_s),p_h]}.
    \]
...
\begin{itemize}
    ...
    \[
    u = \frac{\exp(-\tau_h z) I}{P[\Phi(p_m,p_s),r(z)/A_h]}.
    \]
    \item Bid rent function
    \[
    R_h(z) = A_h\Phi(p_m,p_s)P_2^{-1}\left[\frac{\exp(-\tau_h z) I}{u\Phi(p_m,p_s)}\right].
    \]
    \item For example, with Cobb--Douglas utility,
\[
R_h(z) = A_h\left[\frac{\exp(-\tau_h z) I}{up_m^{\alpha}p_s^{\beta}}\right]^{1/\gamma}.
\]
\end{itemize}
\end{frame}
%\time{2}
...
\begin{itemize}
    ...
    \begin{align*}
    s &= A_s\int_{z_1}^{z_2}\exp(-\tau_s z) dz, \\
    m &= A_m\int_{z_2}^{z_3}\exp(-\tau_m z) dz .
    \end{align*}
    \item Arbitrage at the manufacturing--service boundary $z_2$ pins down relative prices,
    \[
    p_mA_m\exp(-\tau_m z_2) = p_sA_s\exp(-\tau_s z_2),
    \]
