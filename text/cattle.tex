...
To see why this is the case, construct the sectoral bid rent curves $R_i(z)$. This is the maximum rent an active producer in sector $i$ would be willing to pay at location $z$. %In a competitive market, the sector with the highest bid rent curve is active at a location ($R(z)=\max_iR_i(z)$ for all $z$).
...
Agriculture has a special bid-rent curve because $\tau_3=0$ and $\beta_3=1$,
\begin{equation}
...
Assume that $\tau_1/\beta_1>\tau_2/\beta_2>0$. There exists a competitive spatial equilibrium. The structure of this equilibrium is a partitioning, with locations $0<z_1<z_2$ such that services with the highest $\tau_i/\beta_i$ rate locate closest to the CBD in $\left[0,z_1\right]$, manufacturing locates in $\left(z_1,z_2\right]$, and agriculture locates in $(z_2,\infty)$.
\end{proposition}
...
Guess an agricultural price $P_3$. This directly pins down the rural rent $R_3$. 
