...
\]
The LHS is increasing in $z_1$ faster than quadratically. This pins down the equilibrium city size as an increasing function of $N$ and $PA$. Writing out $R_r=P_rA_r$,
\[
	 e^{[z_{1}/(1-\beta)-\tilde z]{\tau}/\beta} L_u 
	=
\beta^{1/(1-\beta)}N
\left( \frac{PA}{P_rA_r} \right)^{1/(1-\beta)}.
\]
...
\frac {PA_c\gamma_c^{1-\beta}} {R_r^\beta W^{1-\beta}}
\]
\[
	N_c =
(1-\beta)^{1/\beta}
