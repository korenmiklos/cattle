...
P_u C_u + P_r C_r
	= Y_c,
\end{equation}
where $Y_c$ is the income of consumer $c$. For tractability, we assume that income is proportional to wages. That is, rent income is rebated to consumers in proportion to their wage earnings.\footnote{This assumption is needed to simplify the analysis of cities with potentially different wages rate due to different amenities. If rents are distributed in other ways, the main analysis remains the same, but equilibrium wages will no longer be simply proportional to amenities.}
...
[NB: formulas simplify if there are no amenities. we can use amenities in calibration, omit them in main model]
...
[NB: If amenities are missing, we can use two equations above to pin down both Ac and Lc. Then we can use data for all cities with population data, not just in the OECD metro database]
