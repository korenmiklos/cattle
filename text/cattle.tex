...
\[
Q_i(z) =\beta_i^{-\beta_i}(1-\beta_i)^{-(1-\beta_i)} A_i L_i(z)^{\beta_i}N_i(z)^{1-\beta_i}.
\]
...
The rural sector, goods can be shipped freely. This will ensure that in equilibrium agricultural goods are produced outside cities in rural areas. 
...
\begin{align}
R_i(z) &=\beta_i^{1-\beta_i}(1-\beta_i)^{-(1-\beta_i)} P_ie^{-\tau_i z}A_i \left(\frac{N_i(z)}{L_i(z)}\right)^{1-\beta_i}\\
W_i &=\beta_i^{-\beta_i}(1-\beta_i)^{\beta_i)} P_ie^{-\tau_i z}A_i \left(\frac{N_i(z)}{L_i(z)}\right)^{-\beta_i}
\end{align}
The sectoral labor-land ratio (employment density) can be expressed as
\begin{equation}
\label{eq:EmpDens}
\frac{N_i(z)}{L_i(z)} = \frac{1-\beta_i}{\beta_i}\frac{R_i(z)}{W_i}.
\end{equation}
...
\begin{equation}
\label{eq:BidRent}
R_i(z) =(P_iA_i)^{1/\beta_i} W_i^{1-1/\beta_i} e^{-\frac{\tau_i}{\beta_i} z}
\end{equation}
...
\[
R_r = (P_rA_r)^{1/\beta_r} W_r^{1-1/\beta_r}
\]
...
\begin{equation}
\label{eq:EmpGrad}
\frac{N_u(z)}{L_u(z)} = \frac{1-\beta_u}{\beta_u} \left(\frac{P_uA_u}{W_u}\right)^{1/\beta_u} e^{-\frac{\tau}{\beta_u} z}.
\end{equation}
...
\begin{equation*}
\tilde{Q}_u=B_u\int_{z=0}^{z_u}e^{-\tau z}A_uL(z)^\beta N_u(z)^{1-\beta}dz,
\end{equation*}
with $B_u=\beta_u^{-\beta_u}(1-\beta_u)^{-(1-\beta_u)}$.
...
\tilde z = -
\frac{\beta_u}{\tau}
\ln\int_{z=0}^{z_u} \frac{L(z)}{L_u}e^{-\frac{\tau}{\beta_u} z}dz
\end{equation}
...
\[
e^{-\frac{\tau}{\beta_u} \tilde z} = \int_{z=0}^{z_u} \frac{L(z)}{L_u}e^{-\frac{\tau}{\beta_u} z}dz.
\]
...
\[
\frac{R_u}{R_r} = \frac{R(0)e^{-\tilde z \tau /\beta_u}} {R_r},
\]
and, given that $R(z_u)=R_r$, 
\[
\frac{R_u}{R_r} = e^{(z_u-\tilde z) \tau /\beta_u}>1.
\]
The ratio of urban to rural rent expenditure is
\[
\frac{R_u L_u}{R_r L_r}=
e^{(z_u-\tilde z) \tau /\beta_u}
\frac{z_u^2 \pi}{L-z_u^2 \pi}.
\]
...
\[
e^{z_{u} \tau} =  
\frac {P_uA_u} {R_r^{\beta_u} W_u^{1-\beta_u}}
\]
\[
 =  
\frac {P_uA_u}{P_rA_r}
...
City employment is
\[
\frac{N_u}{L_u} = \frac{1-\beta_u}{\beta_u} \left(\frac{P_uA_u}{W_u}\right)^{1/\beta_u} e^{-\tilde z\tau/\beta_u}
\] 
\[
=\frac{1-\beta_u}{\beta_u} 
	 \left(\frac{P_uA_u}{P_rA_r}\right)^{1/\beta_u} 
	 \left(\frac{W_r}{P_rA_r}\right)^{-1/\beta_u} 
	 \left(\frac{W_u}{W_r}\right)^{-1/\beta_u} 
	 e^{-\tilde z{\tau/\beta_u}}.
\]	
We want to express everything relative to rural productivity. The rural-urban split is crucial, because there is a wedge: one cannot make a piece of rural land urban. The marginal piece of land is worse than the average urban land (farther from the CBD). Hence cities are better, on average, even if land is optimally allocated.
...
The spatial sorting pins down the supply of urban sectors. We assume that both urban and rural goods are tradable freely within the country. [FIXME: Rewrite tau discussion so that it comes from supply side only.] This yields the law of one price across cities. 

Cities differ in productivity $A_c$ and amenity $\gamma_c$. Rural workers enjoy an amenity of 1.
...
\begin{enumerate}
	...
	\item Utility is equalized across cities. [FIXME: add multiplicative amenities to utility function]
\end{enumerate}
\end{definition}
...
\[
	N_c = L_c
\frac{1-\beta_u}{\beta_u} 
	 \left(\frac{P_uA_c\gamma_c}{P_rA_r}\right)^{1/\beta_u} 
	 \left(\frac{W_r}{P_rA_r}\right)^{-1/\beta_u} 
	 e^{-\tilde z_c{\tau/\beta_u}}
\]
\[
	N_c = \gamma_c 
\frac{1-\beta_u}{\beta_u} 
	 \left(\frac{W_r}{P_rA_r}\right)^{-1/\beta_r} 
	 e^{(z_c-\tilde z_c){\tau/\beta_u}}L_c
\]
[NB: The first condition is much simpler if $\beta_r=\beta_u$. Then city does not depend on wages and average popularion density.]
...
\frac{1-\beta_u}{\beta_u} 
	 \left(\frac{W_r}{P_rA_r}\right)^{-1/\beta_r} 
\sum_c\gamma_c e^{(z_c-\tilde z_c){\tau/\beta_u}}L_c.
\]
Rural labor demand is 
\[
N_r = 
\frac{1-\beta_r}{\beta_r} 
	 \left(\frac{W_r}{P_rA_r}\right)^{-1/\beta_r}L_r.
\]
...
\[
	N_c = \gamma_c N
\frac {e^{(z_c-\tilde z_c){\tau/\beta_u}}L_c}
	{	\frac{(1-\beta_r)/(1-\beta_u)}{\beta_r/\beta_u} 
	L_r+
	\sum_k\gamma_k e^{(z_k-\tilde z_k){\tau/\beta_u}}L_k}
\]
...
\frac{N_c}{N_{c'}} 
= \frac
	 {\left(A_c\gamma_c\right)^{1/\beta_u} e^{-\tilde z_c{\tau/\beta_u}} L_{c}}
	 {\left(A_{c'}\gamma_{c'}\right)^{1/\beta_u} e^{-\tilde z_{c'}{\tau/\beta_u}} L_{c'}}
\]
...
\[
e^{(z_{c}-z_{c'}) \tau} = 
\frac {A_c\gamma_c^{1-\beta_u}}
	{A_{c'}\gamma_{c'}^{1-\beta_u}}.
\]
...
= \frac
	 {\left(N_c/L_c\right)^{\beta_u} e^{\tilde z_c{\tau}}}
	 {\left(N_{c'}/L_{c'}\right)^{\beta_u} e^{\tilde z_{c'}{\tau}}}
\]
\[
\frac {\gamma_c}
	{\gamma_{c'}} = 
\frac
	 {\left(N_c/L_c\right) e^{(\tilde z_c-z_{c}){\tau/\beta_u}}}
	 {\left(N_{c'}/L_{c'}\right) e^{(\tilde z_{c'}-z_{c}){\tau/\beta_u}}}.
\]
\[
\frac{A_c}{A_{c'}} 
= \frac
	 {\left(N_c/L_c\right)^{\beta_u-1} e^{[z_{c}-(1-\beta_u)\tilde z_c]{\tau/\beta_u}}}
	 {\left(N_{c'}/L_{c'}\right)^{\beta_u-1} e^{[z_{c'}-(1-\beta_u)\tilde z_{c'}]{\tau/\beta_u}}}
\]

Urban GDP is a constant multiple of wages,
\[
\frac{1}{1-\beta_u}\sum_c W_cN_c = \frac{1}{1-\beta_u}W_rN\sum_c \frac 1{\gamma_c}N_c/N,
\]
at constant prices,
\[
\frac{P_{u0}}{1-\beta_u}\frac {W_r}{P_u} N_u\sum_c \frac 1{\gamma_c}N_c/N_u.
\]
Substituting in the labor-market clearing for urban labor, constant-price urban GDP is
\[
{P_{u0}}
B_u A_rL_u^{\beta_u} N_u^{1-\beta_u}
	 \left[\sum_c 
	 \left(\gamma_cA_c/A_r\right)^{1/\beta_u} 
	 e^{-\tilde z_c{\tau/\beta_u}} L_{c}/L_u
	 \right]^{\beta_u}
	 \sum_c \frac 1{\gamma_c}N_c/N_u.
\]
...
[NB: if $\beta_u=\beta_r$, the labor market clearing is simpler and city size does not depend on wages.]
