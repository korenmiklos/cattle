...
foreach X in `variables' {
	local graph_`X' "tw "
	forval i=1/3 {
		su `X'_contribution`i' if METRO_ID=="US003", meanonly
	...
	* execute graphing command
	`graph_`X'', scheme(s2mono) legend(order(1 "Services" 2 "Manufacturing" 3 "Agriculture")) xtitle(GDP per capita) ytitle("Relative `X' contribution (Seattle=1)")
	graph export output/`X'.png, width(800) replace
}

tw (lowess z3 gdp_pc) (scatter z3 gdp_pc if METRO_ID=="US003", mlabel(city_name)), legend(off) scheme(s2mono) xtitle(GDP per capita) ytitle("City radius (km)")
